package org.example;// Simple Java Game Engine
// Author: D.P.Playne
...
public abstract class GameEngine implements KeyListener, MouseListener, MouseMotionListener {
	...
	public void setupWindow(int width, int height) {
		...
		mFrame.setTitle("Celesta");
		mFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		mFrame.setLocationRelativeTo(null);//设置窗体居中
		mFrame.setResizable(false);
		mFrame.add(mPanel);
		mFrame.setVisible(true);

		Image logo = (new ImageIcon("ICON.jpg")).getImage();
		mFrame.setIconImage(logo);
...
//	public void drawText(double x, double y, char c, String font, int size) {
//		// Draw text on the screen
//		mGraphics.setFont(new Font(font, Font.PLAIN, size));
//		mGraphics.drawString(Character.toString(c), (int)x, (int)y);
//	}
	...
	public void drawImageWithAlpha(Image image, double x, double y, double w, double h, float alpha) {
		// Check if image is null
		if(image == null) {
			// Print Error message
			System.out.println("Error: cannot draw null image.\n");
		...
		// Draw image on screen at (x,y) with size (w,h) and specified alpha
